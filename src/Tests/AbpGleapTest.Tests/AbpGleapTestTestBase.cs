...
namespace AbpGleapTest.Tests
{
    public abstract class AbpGleapTestTestBase : AbpIntegratedTestBase<AbpGleapTestTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AbpGleapTestTestBase()
        {
        ...
        protected void UsingDbContext(Action<AbpGleapTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpGleapTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpGleapTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpGleapTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpGleapTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpGleapTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpGleapTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpGleapTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpGleapTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpGleapTestDbContext, Task<T>> func)
        {
            T result;
